...
class G15Background:
    ...
    def activate(self):
        ...
        if g15desktop.get_desktop() in ["gnome", "gnome-shell"]:
            self.notify_handlers.append(
                self.gconf_client.notify_add("/desktop/gnome/background/picture_filename", self.config_changed))
            ...
            if self.gnome_dconf_settings is not None:
                self.gnome_dconf_handle = self.gnome_dconf_settings.connect("changed::picture_uri",
    ...
    def _do_config_changed(self):
        ...
        if bg_style is None:
            bg_style = "zoom"
        allow_profile_override = g15gconf.get_bool_or_default(self.gconf_client,
                                                              self.gconf_key + "/allow_profile_override", True)
        ...
        if self.bg_img is None and bg_type == "desktop":
        ...
        if self.bg_img is None:
            self.bg_img = os.path.join(os.path.dirname(__file__),
