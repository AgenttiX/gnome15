...
def show_preferences(parent, driver, gconf_client, gconf_key):
    ...
    g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/raise_on_talk_status_change" % gconf_key,
                                             "RaiseOnTalkStatusChange", False, widget_tree)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/raise_on_chat_message" % gconf_key, "RaiseOnChatMessage",
                                             False, widget_tree)
    dialog.run()
    dialog.hide()
...
class BuddyMenuItem(g15theme.MenuItem):
    ...
    def on_configure(self):
        self.set_theme(
            g15theme.G15Theme(self.parent.get_theme().dir, "menu-entry" if self.group else "menu-child-entry"))
    ...
    def get_theme_properties(self):
        ...
        if self.get_screen().device.bpp == 1:
            ...
            item_properties["item_input_muted_icon"] = g15icontools.get_icon_path(
                MUTED_ICONS if self.input_muted else UNMUTED_ICONS)
            item_properties["item_output_muted_icon"] = g15icontools.get_icon_path(
                "audio-volume-muted" if self.output_muted else "audio-volume-high")
...
class G15Voip(g15plugin.G15MenuPlugin):
    ...
    def action_performed(self, binding):
        ...
        if self.page is not None and self.page.is_visible():
            ...
            if binding.action == g15driver.CLEAR and self.page is not None and self.page.is_visible():
    ...
    def get_theme_properties(self):
        ...
        props["talking_avatar"] = talking_buddy.avatar if talking_buddy is not None else (
            me.avatar if me is not None and me.avatar is not None else self.backend.get_icon())
        ...
        if self.screen.device.bpp == 1:
            ...
            props["status_icon"] = g15icontools.get_icon_path(
                "user-available" if me is not None and not me.away else "user-away")
            props["input_muted_icon"] = g15icontools.get_icon_path(
                MUTED_ICONS if me is not None and me.input_muted else UNMUTED_ICONS)
            props["output_muted_icon"] = g15icontools.get_icon_path(
                "audio-volume-muted" if me is not None and me.output_muted else "audio-volume-high")
    ...
    def talking_status_changed(self, talking):
        ...
        if (self._talking is None and talking is not None) or \
            ...
            if self._talking is not None:
                hex_color = g15gconf.get_string_or_default(self.gconf_client,
...
class KickBuddyMenuItem(BuddyActionMenuItem):
    def __init__(self, buddy, backend):
        BuddyActionMenuItem.__init__(self, 'kick', _('Kick'), buddy, backend,
                                     icon=g15icontools.get_icon_path(['force-exit', 'gnome-panel-force-quit'],
    ...
    def activate(self):
        g15theme.ConfirmationScreen(self.get_screen(), _("Kick Buddy"),
                                    _("Are you sure you want to kick\n%s from the server") % self.buddy.nickname,
                                    self.backend.get_icon(), self._confirm, None)


class BanBuddyMenuItem(BuddyActionMenuItem):
    def __init__(self, buddy, backend):
        BuddyActionMenuItem.__init__(self, 'ban', _('Ban'), buddy, backend, icon=g15icontools.get_icon_path(
            ['audio-volume-muted-blocked', 'mail_spam', 'stock_spam'], include_missing=False))
    ...
    def activate(self):
        g15theme.ConfirmationScreen(self.get_screen(), _("Ban Buddy"),
...
class SelectChannelMenuItem(g15theme.MenuItem):
    def __init__(self, gconf_client, gconf_key, backend, plugin):
        g15theme.MenuItem.__init__(self, 'channel', True, _('Select channel / server'), icon=g15icontools.get_icon_path(
            ['addressbook', 'office-address-book', 'stcok_addressbook', 'x-office-address-book'],
            include_missing=False))
...
class BuddyBacklightMenuItem(BuddyActionMenuItem):
    def __init__(self, gconf_client, gconf_key, buddy, backend, ctrl, plugin):
        BuddyActionMenuItem.__init__(self, 'color', _('Select backlight'), buddy, backend,
                                     icon=g15icontools.get_icon_path(
                                         ['preferences-color', 'gtk-select-color', 'color-picker'],
                                         include_missing=False))
        self._ctrl = ctrl
        self._gconf_client = gconf_client
        self._gconf_key = gconf_key
    ...
    def activate(self):
        BuddyBacklightMenu(self._gconf_client, self._gconf_key, self.get_screen(), self.backend, self.buddy, self._ctrl,
                           self._plugin)


class ReturnMenuItem(g15theme.MenuItem):
    def __init__(self):
        g15theme.MenuItem.__init__(self, 'return', True, _('Back to previous menu'),
                                   icon=g15icontools.get_icon_path(['back', 'gtk-go-back-ltr']))
    ...
    def get_theme_properties(self):
        ...
        if self.get_screen().device.bpp == 1:
            ...
            p["item_icon"] = g15icontools.get_icon_path(
                "audio-volume-muted" if me.output_muted else "audio-volume-high")
...
class OnLineStatusMenuItem(g15theme.MenuItem):
    def __init__(self, backend):
        g15theme.MenuItem.__init__(self, 'online', False, _("Online"),
...
class SelectModeMenuItem(g15theme.MenuItem):
    ...
    def get_theme_properties(self):
        p = g15theme.MenuItem.get_theme_properties(self)
        p["item_radio"] = True
        p["item_radio_selected"] = self._mode == g15gconf.get_string_or_default(self._gconf_client,
...
class ColorMenuItem(BuddyActionMenuItem):
    ...
    def activate(self):
        self._gconf_client.set_string(get_backlight_key(self._gconf_key, self.buddy),
                                      g15convert.rgb_to_string(self.color))
        self.get_root().delete()

    @staticmethod
    def color_square(color, size, radius=0):
