...
class G15RSSPreferences:
    ...
    def __init__(self, parent, driver, gconf_client, gconf_key):
        ...
        self.update_adjustment.set_value(
            g15gconf.get_int_or_default(self._gconf_client, "%s/update_time" % self._gconf_key, 60))
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/twenty_four_hour_times" % gconf_key,
    ...
    def new_url(self, widget):
        ...
        self.feed_list.set_cursor_on_cell(str(len(self.feed_model) - 1), focus_column=self.feed_list.get_column(0),
                                          focus_cell=self.url_renderer, start_editing=True)
...
class G15FeedsMenuItem(g15theme.MenuItem):
    ...
    def get_theme_properties(self):
        ...
        use_twenty_four_hour = g15gconf.get_bool_or_default(self.gconf_client,
...
class G15FeedPage(g15theme.G15Page):
    ...
    def _reload(self):
        ...
        if icon is None:
            icon = g15icontools.get_icon_path(["application-rss+xml", "gnome-mime-application-rss+xml"],
                                              self._screen.height)
    ...
    def _get_theme_properties(self):
        ...
        try:
            ...
            properties["updated"] = "%s %s" % (
            time.strftime("%H:%M", update_time), time.strftime("%a %d %b", update_time))
...
class G15RSS:
    ...
    def activate(self):
        ...
        self._update_time_changed_handle = self._gconf_client.notify_add(self._gconf_key + "/update_time",
    ...
    def _schedule_refresh(self):
        schedule_seconds = g15gconf.get_int_or_default(self._gconf_client, "%s/update_time" % self._gconf_key,
                                                       60) * 60.0
