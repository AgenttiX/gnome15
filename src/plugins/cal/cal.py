...
class CalendarBackend:
    ...
    @staticmethod
    def check_and_add(ve, now, event_days):
        ...
        if self.event.alarm:
            if self.get_screen().device.bpp > 1:
                item_properties["item_icon"] = g15icontools.get_icon_path(
                    ["stock_alarm", "alarm-clock", "alarm-timer", "dialog-warning"])
            else:
...
class Cell(g15theme.Component):
    ...
    def get_theme_properties(self):
        ...
        properties = {
            "weekday": weekday,
            "day": self.day.day,
            "event": self.event.summary if self.event else ""
        }
    ...
    @staticmethod
    def get_item_attributes(selected):
        return {}
...
class G15CalendarPreferences(g15accounts.G15AccountPreferences):
    ...
    def create_general_options(self):
        ...
        g15uigconf.configure_checkbox_from_gconf(self.gconf_client, "%s/twenty_four_hour_times" % self.gconf_key,
...
class G15Cal(g15plugin.G15Plugin):
    def __init__(self, gconf_key, gconf_client, screen):
        ...
        self._icon_path = g15icontools.get_icon_path(
            ["calendar", "evolution-calendar", "office-calendar", "stock_calendar"])
    ...
    def _paint_thumbnail(self, canvas, allocated_size, horizontal):
        if self._page is not None and self._thumb_icon is not None and self.screen.driver.get_bpp() == 16:
