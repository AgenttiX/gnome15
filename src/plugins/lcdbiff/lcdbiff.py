        ...
        if self.refreshing:
            if self.plugin.screen.driver.get_bpp() == 1:
                ...
                item_properties["item_icon"] = g15icontools.get_icon_path(
                    ["view-refresh", "stock_refresh", "gtk-refresh", "view-refresh-symbolic"])
...
class G15Biff(g15plugin.G15MenuPlugin):
    ...
    def activate(self):
        ...
        self.update_time_changed_handle = self.gconf_client.notify_add(self.gconf_key + "/update_time",
    ...
    def schedule_refresh(self, time=- 1):
        ...
        self.refresh_timer = g15scheduler.queue("lcdbiff-%s" % self.screen.device.uid, "MailRefreshTimer", time,
                                                self.refresh)
    ...
    def refresh(self):
        ...
        if self.total_errors > 0:
            ...
            if self.screen.driver.get_bpp() == 1:
                ...
                self.thumb_icon = g15cairo.load_surface_from_file(
                    g15icontools.get_icon_path(["new-messages-red", "messagebox_critical"]))
        else:
            if self.total_count > 0:
                ...
                if self.screen.driver.get_bpp() == 1:
                    ...
                    self.thumb_icon = g15cairo.load_surface_from_file(
                        g15icontools.get_icon_path(["indicator-messages-new", "mail-message-new"]))
            else:
                self._stop_blink()
                self.attention = False
                if self.screen.driver.get_bpp() == 1:
                    ...
                    self.thumb_icon = g15cairo.load_surface_from_file(
                        g15icontools.get_icon_path(["indicator-messages", "mail-message"]))
    ...
    def _paint_panel(self, canvas, allocated_size, horizontal):
        if self.page is not None:
            if self.thumb_icon is not None and self.attention:
                size = g15cairo.paint_thumbnail_image(allocated_size, self.thumb_icon, canvas)
                return size
