...
def show_preferences(parent, driver, gconf_client, gconf_key):
    ...
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/respect_timeout", "RespectTimeout", False,
                                             widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/allow_actions", "AllowActions", False,
                                             widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/allow_cancel", "AllowCancel", True,
                                             widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/on_keyboard_screen", "OnKeyboardScreen", True,
                                             widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/on_desktop", "OnDesktop", True, widget_tree,
                                             True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/blink_keyboard_backlight",
                                             "BlinkKeyboardBacklight", True, widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/blink_memory_bank", "BlinkMemoryBank", True,
                                             widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/change_keyboard_backlight_color",
                                             "ChangeKeyboardBacklightColor", False, widget_tree, True)
    g15uigconf.configure_adjustment_from_gconf(gconf_client, gconf_key + "/blink_delay", "DelayAdjustment", 500,
                                               widget_tree)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/enable_sounds", "EnableSounds", True,
                                             widget_tree, True)
    g15uigconf.configure_colorchooser_from_gconf(gconf_client, gconf_key + "/keyboard_backlight_color",
...
def set_available(widget, widget_tree):
    widget_tree.get_object("KeyboardBacklightColor").set_sensitive(
        widget_tree.get_object("ChangeKeyboardBacklightColor").get_active())
...
class G15Message:
    ...
    def set_details(self, icon, summary, body, timeout, actions, hints):
        self.icon = icon
        self.summary = "None" if summary is None else summary
        ...
        if "image_data" in self.hints:
            ...
            if self.embedded_image is None and (self.icon is None or self.icon == ""):
...
class G15NotifyLCD:
    ...
    def _load_configuration(self):
        self.respect_timeout = g15gconf.get_bool_or_default(self._gconf_client, self._gconf_key + "/respect_timeout",
                                                            False)
        ...
        self.on_keyboard_screen = g15gconf.get_bool_or_default(self._gconf_client,
                                                               self._gconf_key + "/on_keyboard_screen", True)
        self.on_desktop = g15gconf.get_bool_or_default(self._gconf_client, self._gconf_key + "/on_desktop", True)
        self.blink_keyboard_backlight = g15gconf.get_bool_or_default(self._gconf_client,
                                                                     self._gconf_key + "/blink_keyboard_backlight",
                                                                     True)
        self.blink_memory_bank = g15gconf.get_bool_or_default(self._gconf_client,
                                                              self._gconf_key + "/blink_memory_bank", True)
        self.change_keyboard_backlight_color = g15gconf.get_bool_or_default(self._gconf_client,
                                                                            self._gconf_key + "/change_keyboard_backlight_color",
                                                                            False)
        ...
        self.keyboard_backlight_color = g15gconf.get_rgb_or_default(self._gconf_client,
                                                                    self._gconf_key + "/keyboard_backlight_color",
                                                                    (128, 128, 128))
    ...
    def deactivate(self):
        ...
        if self._service:
            if not self._screen.service.shutting_down:
                logger.warning(
                    "Deactivated notify service. Currently the service cannot be reactivated once deactivated. You must completely restart Gnome15")
        ...
        try:
            if self._active:
                ...
                if "x-canonical-private-synchronous" in hints \
                        and (hints["x-canonical-private-synchronous"] == "volume" or hints["x-canonical-private-synchronous"] == "indicator-sound"):
                    return
                ...
                if id != 0 and not id in self._message_map:
                    if len(self._message_queue) > 0:
                        ...
                        logger.warning("Got request to replace message %d, ""but we do not know about it. ""Just replacing visible message %d", id, new_id)
                        id = new_id
                    else:
                        id = 0
    ...
    def _get_theme_properties(self):
        ...
        if self._current_message.icon is not None and len(self._current_message.icon) > 0:
            ...
            if icon_path is None and self._current_message.icon == "notification-message-email":
                icon_path = g15icontools.get_icon_path(
                    ["applications-email-pane", "mail_new", "mail-inbox", "mail-folder-inbox", "evolution-mail"])
        ...
        if not "icon" in properties or properties["icon"] is None:
    ...
    def _notify(self):
        if len(self._message_queue) != 0:
            ...
            if self.enable_sounds and "sound-file" in message.hints and (
                    not "suppress-sound" in message.hints or not message.hints["suppress-sound"]):
            ...
            if control and self.blink_keyboard_backlight:
                acquired_control = self._screen.driver.acquire_control(control, release_after=3.0,
                                                                       val=self.keyboard_backlight_color if self.change_keyboard_backlight_color else control.value)
            ...
            if self.blink_memory_bank:
                acquired_control = self._screen.driver.acquire_control_with_hint(g15driver.HINT_MKEYS,
                                                                                 release_after=3.0,
                                                                                 val=g15driver.MKEY_LIGHT_1 | g15driver.MKEY_LIGHT_2 | g15driver.MKEY_LIGHT_3 | g15driver.MKEY_LIGHT_MR)
                acquired_control.blink(delay=self.blink_delay / 1000.0)

    def _do_redraw(self):
        if self._page is not None:
            self._screen.redraw(self._page)
            self._redraw_timer = g15scheduler.schedule("Notification", self._screen.service.animation_delay,
                                                       self._do_redraw)
