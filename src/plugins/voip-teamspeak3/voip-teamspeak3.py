...
def find_avatar(server_unique_identifier, client_unique_identifier):
    ...
    return os.path.expanduser(
        "~/.ts3client/cache/%s/clients/avatar_%s" % (base64.b64encode(server_unique_identifier), decoded))
...
class Teamspeak3ChannelMenuItem(voip.ChannelMenuItem):
    ...
    def direct_children(self):
        return [child for child in self._backend._channels if
                type(child) is Teamspeak3ChannelMenuItem and child.cpid == self.cid]
...
class Teamspeak3Backend(voip.VoipBackend):
    ...
    def _create_channel_item(self, message, schandlerid):
        item = Teamspeak3ChannelMenuItem(schandlerid, int(message.args['cid']),
                                         int(message.args['cpid']) if 'cpid' in message.args else int(
                                             message.args['pid']),
    ...
    def _parse_notifychanneldeleted_reply(self, message):
        item = self._channel_map[
            int(message.args['cid'][-1] if type(message.args['cid']) is tuple else message.args['cid'])]
        children = self._remove_channel(item)
    ...
    @staticmethod
    def _sort_channellist(channels):
        """
    ...
    def _parse_notifytextmessage_reply(self, message):
        if 'invokername' in message.args and 'msg' in message.args:
            self._plugin.message_received(message.args['invokername'],
                                          self._filter_formatting_tags(message.args['msg']))
        else:
    ...
    @staticmethod
    def _filter_formatting_tags(message):
        filtered = message
