...
class AbstractMPRISPlayer:
    ...
    def schedule_redraw(self):
        self.cancel_redraw()
        self.redraw_timer = g15scheduler.queue("mprisDataQueue-%s" % self.screen.device.uid, "MPRIS2Redraw", 1.0,
                                               self.redraw)
    ...
    def paint_thumbnail(self, canvas, allocated_size, horizontal):
        if self.page is not None and self.thumb_image is not None:
    ...
    def paint_panel(self, canvas, allocated_size, horizontal):
        if self.page is not None and self.thumb_image is not None and self.status == "Playing":
            size = g15cairo.paint_thumbnail_image(allocated_size, self.thumb_image, canvas)
            return size
    ...
    def process_properties(self):
        ...
        if "art_uri" in self.song_properties and self.song_properties["art_uri"] != "":
            ...
            new_cover_uri = None
            if cover_art is not None and os.path.exists(cover_art):
                new_cover_uri = cover_art

        if new_cover_uri is None:
            ...
            if self.cover_uri is not None:
                ...
                if cover_image:
                    ...
                    if cover_image:
                        ...
                        self.cover_image = g15cairo.load_surface_from_file(self.cover_uri,
        ...
        if self.status == "Stopped":
            ...
            self.song_properties["icon"] = g15icontools.get_icon_path(
                ["media-stop", "media-playback-stop", "gtk-media-stop", "player_stop"], self.screen.height)
            ...
            if self.status == "Playing":
                if self.screen.driver.get_bpp() == 1:
                    self.thumb_image = g15cairo.load_surface_from_file(
                        os.path.join(os.path.join(os.path.dirname(__file__), "default"), "play.gif"))
                else:
                ...
                if self.screen.driver.get_bpp() == 1:
                    self.thumb_image = g15cairo.load_surface_from_file(
                        os.path.join(os.path.join(os.path.dirname(__file__), "default"), "pause.gif"))
                else:
                    self.thumb_image = self.cover_image
    ...
    def get_default_cover(self):
        ...
        if mime_type is not None:
            ...
            if mime_icon is not None:
                new_cover_uri = mime_icon
        if new_cover_uri is not None:
            try:
        ...
        if new_cover_uri is None:
            new_cover_uri = g15icontools.get_icon_path(["audio-player", "applications-multimedia"],
                                                       size=self.screen.height)
    ...
    def recalc_progress(self):
        ...
        if 0.0 < self.volume < 34.0:
            vol_icon = "audio-volume-low"
        elif 34.0 <= self.volume < 67.0:
...
class MPRIS1Player(AbstractMPRISPlayer):
    ...
    def __init__(self, gconf_client, screen, players, bus_name, session_bus, theme):
        ...
        self.timer = g15scheduler.queue("mprisDataQueue-%s" % self.screen.device.uid, "UpdateTrackData", 1.0,
                                        self.update_track)
        session_bus.add_signal_receiver(self.track_changed_handler, dbus_interface="org.freedesktop.MediaPlayer",
    ...
    def update_track(self):
        ...
        if self.status == "Playing":
            ...
            self.timer = g15scheduler.queue("mprisDataQueue-%s" % self.screen.device.uid, "UpdateTrackData", 5.0,
                                            self.update_track)

    def on_stop(self):
        if self.timer is not None:
            self.timer.cancel()
        self.session_bus.remove_signal_receiver(self.track_changed_handler,
...
class MPRIS2Player(AbstractMPRISPlayer):
    ...
    def __init__(self, gconf_client, screen, players, bus_name, session_bus, theme):
        ...
        session_bus.add_signal_receiver(self.properties_changed_handler,
                                        dbus_interface="org.freedesktop.DBus.Properties",
                                        signal_name="PropertiesChanged")
        session_bus.add_signal_receiver(self.seeked, dbus_interface="org.mpris.MediaPlayer2.Player",
                                        signal_name="Seeked")
    ...
    def on_stop(self):
        if self.timer:
            self.timer.cancel()
        self.session_bus.remove_signal_receiver(self.properties_changed_handler,
                                                dbus_interface="org.freedesktop.DBus.Properties",
                                                signal_name="PropertiesChanged")
        self.session_bus.remove_signal_receiver(self.seeked, dbus_interface="org.mpris.MediaPlayer2.Player",
                                                signal_name="Seeked")
    ...
    def properties_changed_handler(self, something, properties, list):
        ...
        if self.last_properties is None:
    ...
    def update_track(self):
        ...
        self.reset_elapsed()
        self.timer = g15scheduler.queue("mprisDataQueue-%s" % self.screen.device.uid, "UpdateTrackData",
