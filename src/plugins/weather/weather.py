...
class G15WeatherPreferences:
    ...
    def __init__(self, parent, gconf_client, gconf_key):
        ...
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/use_theme_icons" % gconf_key, "UseThemeIcons", True,
                                                 self._widget_tree)
...
class G15Weather(g15plugin.G15RefreshingPlugin):
    ...
    def refresh(self):
        try:
            ...
            if backend_type:
                backend = get_backend(backend_type).create_backend(self.gconf_client,
    ...
    def _build_properties(self):
        ...
        if self._weather is None:
            ...
            if len(current) == 0:
                ...
                if mono_thumb is not None:
                    attributes["mono_thumb_icon"] = g15cairo.load_surface_from_file(
                        os.path.join(os.path.join(os.path.dirname(__file__), "default"), mono_thumb))
                ...
                if 'forecasts' in self._weather:
                    for forecast in self._weather['forecasts']:
                        ...
                        properties["icon" + str(y)] = g15icontools.get_embedded_image_url(
    ...
    def _translate_icon(self, icon, fallback_icon):
        theme_icon = icon
        if theme_icon is None or theme_icon == "":
            return None
        else:
        ...
        if theme_icon is not None:
            ...
            if icon_path is None and theme_icon.endswith("-night"):
    ...
    def _paint_thumbnail(self, canvas, allocated_size, horizontal):
        ...
        if self.screen.driver.get_bpp() == 1:
            ...
            if "temp" in self._page_properties:
                if horizontal:
                    self._text.set_attributes(self._page_properties["temp"], font_desc="Sans",
