...
class ProcessMenuItem(g15theme.MenuItem):
    ...
    def get_theme_properties(self):
        ...
        item_properties["item_name"] = self.process_name if self.process_name is not None and len(
            self.process_name) > 0 else "Unamed"
    ...
    def activate(self):
        ...
        self.plugin.confirm_screen = g15theme.ConfirmationScreen(self.get_screen(), _("Kill Process"),
                                                                 _("Are you sure you want to kill\n%s") % kill_name,
                                                                 g15icontools.get_icon_path("utilities-system-monitor"),
                                                                 self.plugin._kill_process, self.process_id,
...
class G15Processes(g15plugin.G15MenuPlugin):
    ...
    @staticmethod
    def _get_process_name(args, cmd):
        result = cmd
    ...
    def _do_reload_menu(self):
        ...
        if self.menu.selected is not None and self.menu.get_child_by_id(self.menu.selected.id) is None:
