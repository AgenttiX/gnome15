...
def show_preferences(parent, driver, gconf_client, gconf_key):
    ...
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/keep_aspect_ratio", "KeepAspectRatio", False,
                                             widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/take_over_macro_keys", "TakeOverMacroKeys",
                                             True, widget_tree, True)

    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/use_custom_foreground", "UseCustomForeground",
                                             False, widget_tree)
    g15uigconf.configure_colorchooser_from_gconf(gconf_client, gconf_key + "/custom_foreground", "CustomForeground",
...
class G15DaemonClient(asyncore.dispatcher):
    def __init__(self, conn, plugin):
        ...
        self.page = g15theme.G15Page("G15Daemon%d" % self.plugin.screen_index, plugin.screen, painter=self._paint,
                                     on_shown=self._on_shown, on_hidden=self._on_hidden, originating_plugin=self)
        ...
        if val == CLIENT_CMD_SWITCH_PRIORITIES:
            ...
            if isinstance(self.plugin.default_backlight, int):
                ...
                if level == 0:
                    ...
                    bl = (self.plugin.default_backlight[0] / 2, self.plugin.default_backlight[1] / 2,
                          self.plugin.default_backlight[2] / 2)
                elif level == 2:
            ...
            if self.keyboard_backlight_acquire:
                ...
                logger.warning(
                    "g15daemon client requested keyboard backlight be changed, but there is no backlight to change")
    ...
    def _on_shown(self):
        ...
        if self.backlight_control:
            ...
            self.backlight_acquire = self.plugin.screen.driver.acquire_control(self.backlight_control,
                                                                               val=self.backlight_value)
        ...
        if self.keyboard_backlight_control:
            ...
            self.keyboard_backlight_acquire = self.plugin.screen.driver.acquire_control(self.keyboard_backlight_control,
                                                                                        val=self.keyboard_backlight_value)
...
class G15DaemonServer:
    ...
    def _get_port(self):
        ...
        return 15550 if port_entry is None else port_entry.get_int()

    def _config_changed(self, client, connection_id, entry, args):
        self.load_configuration()
        self.screen.redraw()
        port = self._get_port()
        if self.daemon is None or self.daemon.port != port:
            if self.daemon is not None:
                logger.warning("Port changed to %d (will restart daemon - clients may have to be reconnected manually",
                               port)
                self._stop_all_clients()
                self.daemon.close()
    ...
    def load_configuration(self):
        self.take_over_macro_keys = g15gconf.get_bool_or_default(self.gconf_client,
                                                                 "%s/take_over_macro_keys" % self.gconf_key, True)

        if g15gconf.get_bool_or_default(self.gconf_client, "%s/use_custom_foreground" % self.gconf_key, False):
            col = g15gconf.get_rgb_or_default(self.gconf_client, "%s/custom_foreground" % self.gconf_key,
                                              (255, 255, 255))
