...
class G15Menu(g15plugin.G15MenuPlugin):
    ...
    def __init__(self, gconf_client, gconf_key, screen):
        g15plugin.G15MenuPlugin.__init__(self, gconf_client, gconf_key, screen,
                                         ["gnome-main-menu", "gnome-window-manager", "gnome-gmenu", "kmenuedit"], id,
                                         name)
    ...
    def _load_item_icon(self, item):
        if item._item_page.thumbnail_painter is not None:
...
class MenuScreenChangeListener(g15screen.ScreenChangeAdapter):
    ...
    def new_page(self, page):
        if self.plugin.page is not None and page != self.plugin.page and page.priority > g15screen.PRI_INVISIBLE:
    ...
    def title_changed(self, page, title):
        if self.plugin.page is not None and page != self.plugin.page:
            self.plugin.page.redraw()
