...
class G15MacroRecorder:
    ...
    def _done_recording(self, state):
        if self._record_keys is not None:
    ...
    def _get_theme_properties(self):
        ...
        properties = {
            "icon": g15icontools.get_icon_path(self.icon, self._screen.height),
            "memory": "M%d" % self._screen.get_memory_bank()}
