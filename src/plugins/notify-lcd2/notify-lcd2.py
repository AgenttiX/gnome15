...
def show_preferences(parent, gconf_client, gconf_key):
    ...
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/allow_actions", "AllowActions", False,
                                             widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/allow_cancel", "AllowCancel", False,
                                             widget_tree, True)
    ...
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/blink_keyboard", "BlinkKeyboard", False,
                                             widget_tree, True)
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/change_keyboard_color", "ChangeKeyboardColor",
                                             False, widget_tree, True)
    g15uigconf.configure_adjustment_from_gconf(gconf_client, gconf_key + "/blink_delay", "DelayAdjustment", 500,
                                               widget_tree)
    g15uigconf.configure_colorchooser_from_gconf(gconf_client, gconf_key + "/color", "Color", (128, 128, 128),
                                                 widget_tree, None)
...
class G15NotifyService(dbus.service.Object):
    ...
    def _get_enable_sounds(self):
        enable_sounds = self._gconf_client.get(self._gconf_key + "/enable_sounds")
        return enable_sounds is None or enable_sounds.get_bool()
    ...
    def _notify(self):
        if len(self._message_queue) != 0:
            ...
            # Play sound
            if self._get_enable_sounds() and "sound-file" in message.hints and (
            ...
            if self._gconf_client.get_bool(self._gconf_key + "/change_keyboard_color"):
                ...
                self._driver.BlinkKeyboard(0.0, 3.0, (128, 128, 128) if color is None else g15convert.to_rgb(color))
...
class G15NotifyLCD:
    ...
    def activate(self):
        ...
        for i in range(0, 6):
            try:
                for pn in OTHER_NOTIFY_DAEMON_PROCESS_NAMES:
                    ...
                    process.wait()
                self._bus_name = dbus.service.BusName(IF_NAME, bus=self._bus, replace_existing=True,
                                                      allow_replacement=True, do_not_queue=True)
                break
        ...
        self._notification_service = G15NotifyService(self._gconf_client, self._gconf_key, self._service, self._driver,
                                                      self._bus)
        try:
