...
class VolumeMenuItem(g15theme.MenuItem):
    ...
    def get_theme_properties(self):
        ...
        item_properties["item_icon"] = g15icontools.get_icon_path(
            [self.volume.get_icon().get_names()[0], "gnome-dev-harddisk"])
        return item_properties
...
class G15Places(g15plugin.G15MenuPlugin):
    ...
    def _on_mount_removed(self, monitor, mount, *args):
        ...
        for volume in self.volume_monitor.get_volumes():
            if not self._get_item_for_volume(volume) and volume.get_mount() is None:
                self._add_volume(volume)

        self._popup()

    def _popup(self):
        if not self.page.is_visible() and g15gconf.get_bool_or_default(self.gconf_client, "%s/raise" % self.gconf_key,
                                                                       True):
