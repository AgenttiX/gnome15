...
class NOAAWeatherOptions(weather.WeatherOptions):
    def __init__(self, gconf_client, gconf_key):
        ...
        g15uigconf.configure_text_from_gconf(gconf_client, "%s/station_id" % gconf_key, "StationID", "KPEO",
                                             self.widget_tree)
...
class NOAAWeatherBackend(weather.WeatherBackend):
    ...
    def get_weather_data(self):
        ...
        data = {
            ...
            "current_conditions": {
                "wind_speed": g15pythonlang.to_int_or_none(
                    weather.mph_to_kph(float(p["wind_mph"]))) if "wind_mph" in p else None,
                ...
                "fallback_icon": "http://w1.weather.gov/images/fcicons/%s" % (
                            "%s.jpg" % os.path.splitext(p["icon_url_name"])[0]) if "icon_url_name" in p else None
            }
        }
