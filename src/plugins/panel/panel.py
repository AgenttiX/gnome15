...
class G15PanelPainter(g15screen.Painter):
    ...
    def paint(self, canvas):
        ...
        if position == "top" or position == "bottom":
            ...
            for page in self.screen.pages:
                if page != self.screen.get_visible_page() and page.panel_painter is not None:
                    if actual_size > 0:
                        panel_canvas.translate(inset + gap, 0)
                        actual_size += inset + gap
                    panel_canvas.save()
                    panel_canvas.set_source_rgb(
                        *self.screen.driver.get_color_as_ratios(g15driver.HINT_FOREGROUND, (0, 0, 0)))
        ...
        if position == "bottom":
            canvas.translate(0 if align == "start" else self.screen.width - actual_size - gap,
                             self.screen.height - panel_height)
        elif position == "right":
            canvas.translate(self.screen.width - panel_height,
                             0 if align == "start" else self.screen.height - actual_size - gap)
        elif position == "top":
    ...
    def _get_panel_position(self):
        ...
        if panel_pos is None or panel_pos == "":
            panel_pos = "bottom"
        return panel_pos
