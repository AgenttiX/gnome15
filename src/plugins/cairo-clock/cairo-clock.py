...
class G15CairoClockPreferences:
    ...
    def __init__(self, parent, driver, gconf_key, gconf_client):
        ...
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/display_seconds" % gconf_key,
                                                 "DisplaySecondsCheckbox", True, widget_tree)
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/display_date" % gconf_key, "DisplayDateCheckbox",
                                                 True, widget_tree)
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/twenty_four_hour" % gconf_key,
                                                 "TwentyFourHourCheckbox", True, widget_tree)
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/display_digital_time" % gconf_key,
                                                 "DisplayDigitalTimeCheckbox", True, widget_tree)
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/display_year" % gconf_key, "DisplayYearCheckbox",
                                                 True, widget_tree)
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/second_sweep" % gconf_key, "SecondSweep", False,
                                                 widget_tree)
        g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/twenty_four_hour_digital" % gconf_key,
...
class G15CairoClock(g15plugin.G15RefreshingPlugin):
    ...
    def get_next_tick(self):
        ...
        if self.second_sweep:
            ...
            next_tick = datetime.datetime(next_tick.year, next_tick.month, next_tick.day, next_tick.hour,
                                          next_tick.minute, int(next_tick.second))
        else:
    ...
    def _load_surfaces(self):
        ...
        self.display_seconds = g15gconf.get_bool_or_default(self.gconf_client, "%s/display_seconds" % self.gconf_key,
                                                            True)
        self.display_date = g15gconf.get_bool_or_default(self.gconf_client, "%s/display_date" % self.gconf_key, True)
        self.display_year = g15gconf.get_bool_or_default(self.gconf_client, "%s/display_year" % self.gconf_key, True)
        self.display_digital_time = g15gconf.get_bool_or_default(self.gconf_client,
                                                                 "%s/display_digital_time" % self.gconf_key, True)
        self.second_sweep = g15gconf.get_bool_or_default(self.gconf_client, "%s/second_sweep" % self.gconf_key, False)
        self.twenty_four_hour = g15gconf.get_bool_or_default(self.gconf_client, "%s/twenty_four_hour" % self.gconf_key,
                                                             False)
        self.twenty_four_hour_digital = g15gconf.get_bool_or_default(self.gconf_client,
                                                                     "%s/twenty_four_hour_digital" % self.gconf_key,
                                                                     True)
        ...
        self.clock_theme_dir = get_theme_dir(self.screen.driver.get_model_name(), self.gconf_key, self.gconf_client,
                                             theme)
    ...
    def _load_surface_list(self, names):
        ...
        for i in names:
            ...
            if os.path.exists(path):
                ...
                surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, img_surface.get_width() * 2,
    ...
    def _do_paint(self, canvas, width, height, draw_date=True, draw_time=True):
        canvas.save()
        self._do_paint_clock(canvas, width, height, draw_date and self.screen.driver.get_bpp() != 1,
                             draw_time and self.screen.driver.get_bpp() != 1)
        canvas.restore()
        ...
        if self.screen.driver.get_bpp() == 1:
            ...
            if draw_date:
                ...
                self.time_text.set_attributes(pxwidth=56, text=date_text, align=pango.ALIGN_CENTER, font_desc="Fixed",
                                              font_absolute_size=12 * pango.SCALE)
                self.time_text.draw(0, 15)

            if draw_time:
                time_text = self._get_time_text()
                self.time_text.set_attributes(pxwidth=56, text=time_text, align=pango.ALIGN_CENTER, font_desc="Fixed",
                                              font_absolute_size=12 * pango.SCALE)
    ...
    @staticmethod
    def _draw_hand(drawing_context, hand_surfaces, width, height, deg):
