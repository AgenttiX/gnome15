...
class G15SensorsPreferences:
    ...
    def __init__(self, parent, driver, gconf_client, gconf_key):
        ...
        self.interval_adjustment.set_value(
            g15gconf.get_float_or_default(self._gconf_client, "%s/interval" % self._gconf_key, 10))
        ...
        if value != "":
            if self.sensor_model[row_index][2] != value:
                ...
                self._gconf_client.set_string(
                    "%s/sensors/%s/label" % (self._gconf_key, gconf.escape_key(sensor_name, len(sensor_name))), value)
    ...
    def sensor_toggled(self, widget, row_index):
        ...
        self._gconf_client.set_bool(
            "%s/sensors/%s/enabled" % (self._gconf_key, gconf.escape_key(sensor_name, len(sensor_name))), now_active)
        ...
        for source in ss:
            ...
            for sensor in sa:
                ...
                if sensor.sense_type in TYPE_NAMES:
                    self.sensor_model.append([sensor.name, g15gconf.get_bool_or_default(self._gconf_client,
                                                                                        "%s/enabled" % sense_key,
                                                                                        True),
                                              g15gconf.get_string_or_default(self._gconf_client,
                                                                             "%s/label" % sense_key, sensor.name),
                                              TYPE_NAMES[sensor.sense_type]])
            source.stop()
        ...
        for device in self.udisks.EnumerateDevices():
            ...
            if udisk_properties.Get(UDISKS_DEVICE_NAME, "DriveAtaSmartIsAvailable"):
                ...
                if int(udisk_properties.Get(UDISKS_DEVICE_NAME, "DriveAtaSmartTimeCollected")) > 0:
                    ...
                    process = subprocess.Popen(['skdump', '--temperature', '--load=-'], shell=False,
