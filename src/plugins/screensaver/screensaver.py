...
name = _("Screensaver")
description = _(
    "Dim the keyboard and display a message (on models with an LCD screen) when the desktop screen saver activates.")
...
def show_preferences(parent, driver, gconf_client, gconf_key):
    ...
    g15uigconf.configure_checkbox_from_gconf(gconf_client, "%s/dim_keyboard" % gconf_key, "DimKeyboardCheckbox", True,
                                             widget_tree)
...
class G15ScreenSaver:
        ...
        if self._session_bus is None:
            ...
            for dbus_name, interface, path in screensavers:
                try:
                    ...
                    self._session_bus.add_signal_receiver(self._screensaver_changed_handler,
    ...
    def destroy(self):
        if self._session_bus:
            self._session_bus.remove_signal_receiver(self._screensaver_changed_handler,
                                                     dbus_interface=self._dbus_interface, signal_name="ActiveChanged")
    ...
    def _check_page(self):
        if self._in_screensaver:
            ...
            if not self.dimmed and g15gconf.get_bool_or_default(self._gconf_client, "%s/dim_keyboard" % self._gconf_key,
                                                                True):
                self._dim_keyboard()
        else:
            if self._screen.driver.get_bpp() != 0:
                self._remove_page()
            if self.dimmed and g15gconf.get_bool_or_default(self._gconf_client, "%s/dim_keyboard" % self._gconf_key,
                                                            True):
                self._light_keyboard()
