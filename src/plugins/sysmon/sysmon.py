...
def show_preferences(parent, driver, gconf_client, gconf_key):
    ...
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/show_cpu_on_panel", "ShowCPUUsageOnPanel",
                                             True, widget_tree)
    dialog.run()
    dialog.hide()
...
class Net:
    ...
    def new_data(self, this_net_list):
        ...
        if self.last_net_list is not None:
...
class G15SysMon(g15plugin.G15RefreshingPlugin):
    ...
    def _paint_thumbnail(self, canvas, allocated_size, horizontal):
        if self.page is not None and self._thumb_icon is not None and self.screen.driver.get_bpp() == 16:
    ...
    def _paint_panel(self, canvas, allocated_size, horizontal):
        if self.page is not None and self.screen.driver.get_bpp() == 16:
            canvas.save()
