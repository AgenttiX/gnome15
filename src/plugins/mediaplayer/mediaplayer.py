...
g15devices.g15_action_keys[NEXT_TRACK] = g15actions.ActionBinding(NEXT_TRACK, [g15driver.G_KEY_NEXT],
                                                                  g15driver.KEY_STATE_UP)
g15devices.z10_action_keys[NEXT_TRACK] = g15actions.ActionBinding(NEXT_TRACK, [g15driver.G_KEY_NEXT],
                                                                  g15driver.KEY_STATE_UP)
g15devices.g19_action_keys[NEXT_TRACK] = g15actions.ActionBinding(NEXT_TRACK, [g15driver.G_KEY_NEXT],
                                                                  g15driver.KEY_STATE_UP)
g15devices.g15_action_keys[PREV_TRACK] = g15actions.ActionBinding(PREV_TRACK, [g15driver.G_KEY_PREV],
                                                                  g15driver.KEY_STATE_UP)
g15devices.z10_action_keys[PREV_TRACK] = g15actions.ActionBinding(PREV_TRACK, [g15driver.G_KEY_PREV],
                                                                  g15driver.KEY_STATE_UP)
g15devices.g19_action_keys[PREV_TRACK] = g15actions.ActionBinding(PREV_TRACK, [g15driver.G_KEY_PREV],
                                                                  g15driver.KEY_STATE_UP)
g15devices.g15_action_keys[STOP_TRACK] = g15actions.ActionBinding(STOP_TRACK, [g15driver.G_KEY_STOP],
                                                                  g15driver.KEY_STATE_UP)
g15devices.z10_action_keys[STOP_TRACK] = g15actions.ActionBinding(STOP_TRACK, [g15driver.G_KEY_STOP],
                                                                  g15driver.KEY_STATE_UP)
g15devices.g19_action_keys[STOP_TRACK] = g15actions.ActionBinding(STOP_TRACK, [g15driver.G_KEY_STOP],
                                                                  g15driver.KEY_STATE_UP)
g15devices.g15_action_keys[PLAY_TRACK] = g15actions.ActionBinding(PLAY_TRACK, [g15driver.G_KEY_PLAY],
                                                                  g15driver.KEY_STATE_UP)
g15devices.z10_action_keys[PLAY_TRACK] = g15actions.ActionBinding(PLAY_TRACK, [g15driver.G_KEY_PLAY],
                                                                  g15driver.KEY_STATE_UP)
g15devices.g19_action_keys[PLAY_TRACK] = g15actions.ActionBinding(PLAY_TRACK, [g15driver.G_KEY_PLAY],
                                                                  g15driver.KEY_STATE_UP)
...
unsupported_models = [g15driver.MODEL_G930, g15driver.MODEL_G35, g15driver.MODEL_G110, g15driver.MODEL_G11,
                      g15driver.MODEL_G11, g15driver.MODEL_MX5500]
...
icon_path = g15icontools.get_icon_path(
    ["media-video", "emblem-video", "emblem-videos", "video", "video-player", "applications-multimedia"])
...
class G15MediaPlayerPage(g15theme.G15Page):
    ...
    def __init__(self, screen, source, plugin):
        g15theme.G15Page.__init__(self, "videopage-%s" % source.name, screen,
                                  ...
                                  title=source.name,
                                  theme=g15theme.G15Theme(self, variant='mediakeys' if plugin._grabbed_keys else None),
                                  thumbnail_painter=self._paint_thumbnail, originating_plugin=plugin)
            ...
            if can_grab_media_keys:
                ...
                if binding.action == g15driver.SELECT:
                    gobject.idle_add(self._play)
                        ...
                        binding.action == g15driver.PREVIOUS_PAGE and self._screen.device.model_id == g15driver.MODEL_G19) or \
                        (
    ...
    @staticmethod
    def _on_sync_message(bus, message):
    ...
    def _paint_thumbnail(self, canvas, allocated_size, horizontal):
        if self._surface is not None and self._screen.driver.get_bpp() == 16:
...
class G15MediaPlayer(g15plugin.G15MenuPlugin):
    ...
    def load_menu_items(self):
        ...
        if len(video_devices) > 0:
            items.append(g15theme.MenuItem("video-devices", True, _("Video Devices"),
                                           icon=g15icontools.get_icon_path(["camera-web", "camera-video"]),
                                           activatable=False))
        ...
        items.append(g15theme.MenuItem("video-file", True, _("Open Audio/Video File"), activate=activate_video_file,
        ...
        if len(removable_media_items):
            items.append(g15theme.MenuItem("removable-devices", True, _("Removable Devices"),
                                           icon=g15icontools.get_icon_path(
                                               ["driver-removable-media", "gnome-dev-removable"]), activatable=False))
        ...
        if status == 0 and len(output) > 0:
            ...
            if len(pulse_items) > 0:
                items.append(g15theme.MenuItem("pulse-sources", True, _("PulseAudio Source"),
                                               icon=g15icontools.get_icon_path(
                                                   ["audio-card", "audio-speakers", "audio-volume-high",
                                                    "audio-x-generic"]), activatable=False))
                items += pulse_items

        # Visualisations - TODO - there must be a better way to list them
        items.append(g15theme.MenuItem("visualisation-mode", True, _("Visualisation Mode"),
                                       icon=g15icontools.get_icon_path(
                                           ["preferences-color", "gtk-select-color", "preferences-desktop-screensaver",
                                            "kscreensaver", "xscreensaver"]), activatable=False))
        for c in ["goom",
                  "libvisual_bumpscope",
                  "libvisual_corona",
                  "libvisual_infinite",
