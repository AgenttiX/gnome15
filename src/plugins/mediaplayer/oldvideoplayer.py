...
unsupported_models = [g15driver.MODEL_G110, g15driver.MODEL_G11, g15driver.MODEL_G11, g15driver.MODEL_MX5500,
...
class PlayThread(Thread):
    def __init__(self, page):
        ...
        self._process = subprocess.Popen(['mplayer', '-slave', '-noconsolecontrols', '-really-quiet',
                                          '-vo', 'jpeg', self._page._movie_path], cwd=self.temp_dir,
...
class G15VideoPage(g15theme.G15Page):
    ...
    def paint_theme(self, canvas, properties, attributes):
        ...
        if 0 > self._sidebar_offset > -(self.theme.bounds[2]):
    ...
    def _paint_thumbnail(self, canvas, allocated_size, horizontal):
        if self._thumb_icon is not None and self._screen.driver.get_bpp() == 16:
...
class G15VideoPlayer():
    ...
    def deactivate(self):
        if self._page._playing is not None:
            self._page._stop()
    ...
    def action_performed(self, binding):
        if self._page is not None and self._page.is_visible():
            if binding.action == g15driver.SELECT:
                ...
                if self._page._playing is None:
                    self._page._play()
