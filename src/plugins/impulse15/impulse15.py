...
def show_preferences(parent, driver, gconf_client, gconf_key):
    ...
    g15uigconf.configure_checkbox_from_gconf(gconf_client, gconf_key + "/animate_mkeys", "AnimateMKeys", False,
                                             widget_tree)
    ...
    g15uigconf.configure_combo_from_gconf(gconf_client, gconf_key + "/audio_source_name", "AudioSource", source_name,
                                          widget_tree)
    ...
    g15uigconf.configure_colorchooser_from_gconf(gconf_client, gconf_key + "/col1", "Color1", (255, 0, 0), widget_tree,
                                                 default_alpha=255)
    g15uigconf.configure_colorchooser_from_gconf(gconf_client, gconf_key + "/col2", "Color2", (0, 0, 255), widget_tree,
                                                 default_alpha=255)
    g15uigconf.configure_adjustment_from_gconf(gconf_client, gconf_key + "/frame_rate", "FrameRateAdjustment", 10.0,
                                               widget_tree)
...
class G15ImpulsePainter(g15screen.Painter):
    ...
    def _set_mkey_lights(self, val):
        if val > 200:
            self.mkey_acquisition.set_value(
                g15driver.MKEY_LIGHT_MR | g15driver.MKEY_LIGHT_1 | g15driver.MKEY_LIGHT_2 | g15driver.MKEY_LIGHT_3)
        elif val > 100:
...
class G15Impulse:
    ...
    def _load_config(self):
        ...
        self.refresh_interval = 1.0 / g15gconf.get_float_or_default(self.gconf_client, self.gconf_key + "/frame_rate",
                                                                    25.0)
        ...
        if paint != self.last_paint and self.screen.driver.get_bpp() != 0:
            ...
            if paint == "screen":
                if self.page is None:
                    self.page = g15theme.G15Page(id, self.screen, title=name, painter=self.painter.paint,
