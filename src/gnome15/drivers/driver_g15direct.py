...
class Driver(g15driver.AbstractDriver):
    ...
    @staticmethod
    def _get_g510_multimedia_keys(code):
        keys = []
        code &= ~(1 << 28)
        if code & 1 << 0 != 0:
    ...
    @staticmethod
    def _convert_ext_g15daemon_code(code):
        keys = []
    ...
    @staticmethod
    def _convert_from_g15daemon_code(code):
        keys = []
    ...
    @staticmethod
    def _emit_macro_keys(this_keys, pos, low_val, high_val):
        if pos[0] < low_val:
