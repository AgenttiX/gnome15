...
class G15GlobalConfig:
    ...
    def run(self):
        ...
        elif self._get_selected_plugin() is None:
...
class G15Config:
    ...
    def _show_message(self, type, text, start_service_button=True):
        ...
        if self.start_button is not None and not start_service_button:
            self.start_button.hide()
        self.warning_box_shown = True

    @staticmethod
    def _open_site(widget):
        subprocess.Popen(['xdg-open', widget.get_uri()])

    @staticmethod
    def _to_rgb(string_rgb):
    ...
    def _load_macro_state(self):
        ...
        self.memory_bank.set_visible(self.driver is not None and self.driver.has_memory_bank())
    ...
    def _load_plugins(self):
        ...
        if self.selected_device:
            ...
            if len(self.plugin_model) > 0 and self._get_selected_plugin() is None:
    ...
    @staticmethod
    def _macro_profile_dropped(widget, context, x, y, selection, info, timestamp):
    ...
    def _set_enabled_value_from_configuration(self):
        enabled = g15devices.is_enabled(self.conf_client,
                                        self.selected_device) if self.selected_device is not None else False
        self.device_enabled.set_active(enabled)
        self.device_enabled.set_sensitive(self.selected_device is not None)
    ...
    def _set_available_actions(self):
        ...
        self.delete_macro_button.set_sensitive(path is not None and not self.selected_profile.read_only)
        self.macro_properties_button.set_sensitive(path is not None)
    ...
    def _load_profile_list(self):
        ...
        if self.selected_device is not None:
            ...
            for profile in self.profiles:
                ...
                if current_selection is not None and profile.id == current_selection.id:
    ...
    def _load_profile(self, profile):
        ...
        try:
            ...
            for macro in macros:
                ...
                if current_selection is not None and macro.key_list_key == current_selection.key_list_key:
                    tree_selection.select_path(
                        self.macros_model.get_path(self.macros_model.get_iter(len(self.macros_model) - 1)))
                    selected_macro = macro
            if selected_macro is None and len(macros) > 0:
            ...
            if profile.window_name is not None:
            ...
            for i in range(0, len(self.parent_profile_model)):
                if (profile.base_profile is None and i == 0) or \
    ...
    def _load_windows(self):
        ...
        if self.bamf_matcher is not None:
            for window in self.bamf_matcher.RunningApplications():
                ...
                if window_name is not None and vn == window_name:
            ...
            for app in apps:
                ...
                if window_name is not None and app == window_name:
                    self.window_combo.set_active(i)
                i += 1
    ...
    def _add_controls(self):
        ...
        if self.driver is not None and self.driver.get_bpp() == 0:
        ...
        control = self.driver.get_control_for_hint(g15driver.HINT_DIMMABLE) if self.driver is not None else None
        if control is not None and not isinstance(control.value, int):
            self._controls_visible = True
            hbox = gtk.HBox()
