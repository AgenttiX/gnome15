...
def create_profile(profile):
    ...
    """
    if profile.id is None or profile.id == -1:
...
class G15Profile(object):
    ...
    def are_keys_in_use(self, activate_on, memory, keys, exclude=None):
        ...
        for macro in bank:
            if (exclude is None or (exclude is not None and not self._is_excluded(exclude, macro))) and sorted(
    ...
    @staticmethod
    def _get_section_name(state, memory):
    ...
    @staticmethod
    def _is_excluded(excluded, macro):
        for e in excluded:
            if e == macro:
                return True
