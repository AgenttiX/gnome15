...
class Component(object):
    ...
    def is_visible(self):
        return self.parent is not None and self.parent.is_visible()
...
class Menu(Component):
    ...
    @staticmethod
    def get_item_height(item, group=False):
    ...
    def select_first(self):
        self.get_tree_lock().acquire()
        try:
            if not self.selected is None and not self.contains_child(self.selected):
                self.selected = None
...
class DBusMenu(Menu):
    ...
    def menu_changed(self, menu=None, property=None, value=None):
        ...
        if menu is not None:
            if property is not None and property == dbusmenu.VISIBLE and value and menu.type != "separator":
                self.selected = menu
        else:
            # Layout change
...
class G15Theme(object):
        ...
        try:
            ...
            if self.page is None:
                ...
                if self.dir is not None:
                    ...
                    prefix_path = self.prefix if self.prefix is not None else os.path.basename(
                ...
                elif self.svg_text is not None:
    ...
    def draw(self, canvas, properties={}, attributes={}):
        if self.render is not None and self.auto_dirty:
        ...
        if self.render is None or self.dirty:
            self.render_lock.acquire()
            ...
            try:
                ...
                if self.svg_processor is not None:
    ...
    def _set_relative_image_paths(self, root):
        for element in root.xpath('//svg:image[@xlink:href]', namespaces=self.nsmap):
            ...
            is_abs = href and (href.startswith("http:") or href.startswith("https:") or href.startswith(
                "file:") or href.startswith("/"))
    ...
    def _convert_image_urls(self, root, properties):
        ...
        for element in root.xpath('//svg:image', namespaces=self.nsmap):
            ...
            if id is not None and id in properties and properties[id] is not None:
                file_str = StringIO()
                val = properties[id]
    ...
    @staticmethod
    def _get_actual_size(element, width, height):
